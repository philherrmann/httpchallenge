# nb: no easy way of sniffing HTTPS since encrypted end-2-end (or checkout TCP)
# either MITM or check TCP connections on specific port 443

from logging import getLogger
from typing import Callable
from threading import Event
from dataclasses import dataclass
from abc import ABC, abstractmethod
import time



from src.collectors.basic_collector import BasicCollector
from src.sniffers.snappy_sniffer import SnappySniffer
from src.views.print_view import PrintView
from src.types.http_info import HTTPInfo


logger = getLogger("HTTPSniffer")


class MyController:

    def __init__(self):
        self.http_collector = BasicCollector()
        self.sniffer = SnappySniffer(receive_http_callback=lambda http_info: self._receive_http_callback(http_info))
        self.view = PrintView()
        self.stop_event = Event()

    def _receive_http_callback(self, http_info: HTTPInfo):
        self.http_collector.collect_http_info(http_info)

    def start(self):
        self.sniffer.start()
        while not self.stop_event.is_set():
            time.sleep(10)
            self._update_view()

    def _update_view(self):
        current_results = dict(self.http_collector.http_container)
        for k, v in current_results.items():
            self.view.update(k)
        self.view.update(f"Total Length: {self.http_collector.total_content_length}")
        self.http_collector.clear()

    def stop(self):
        self.stop_event.set()
        self.sniffer.stop()


def main():

    '''
    sniffer = MySniffer()
    sniffer.start()
    time.sleep(60)
    sniffer.stop()
    '''
    controller = MyController()
    controller.start()
    time.sleep(30)
    controller.stop()


if __name__ == '__main__':
    main()
